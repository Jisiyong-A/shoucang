...
const SERVER_NAME = 'shoucang-notes';
...
const tools = [
  {
    ...
    description: '搜索“收藏”里已经保存到本机的笔记。覆盖标题、正文、图片 OCR、标签、作者和分类；不会访问小红书账号或网络。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  switch (message.method) {
    case 'initialize':
      return {
        ...
        instructions: '这是“收藏”的本地只读笔记库。需要相关资料时先调用 search_saved_notes，再按需调用 read_saved_note。不得把这些工具描述成小红书账号访问；它们只读取用户已经保存到本机的内容。',
      };
    case 'ping':
      return {};
    case 'tools/list':
