...
const mcpServerPath = fileURLToPath(new URL('../shoucang-mcp.mjs', import.meta.url));

test('MCP server searches and reads saved local notes over stdio', async (t) => {
  const dataDirectory = await mkdtemp(path.join(os.tmpdir(), 'shoucang-mcp-'));
  ...
  assert.equal(initialized.result.serverInfo.name, 'shoucang-notes');
