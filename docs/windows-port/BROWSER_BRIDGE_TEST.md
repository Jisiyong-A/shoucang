...
|---|---|---|---|---|
| **Chrome**（Chrome for Testing 151.0.7922.138，`--load-extension` 加载） | ⚠️ 未单独测卡片拖拽源（按钮 dragstart 同一 dataTransfer 逻辑） | ✅ **实测**：真实笔记页点击「拖到看看收藏」→ 导入成功（notes+1, mediaStatus=ready） | ✅ **App 侧实测**（见下 Drag 说明） | ✅ 扩展按钮显示 `LOCAL ENGINE OFFLINE`，页面 readyState=complete 不挂死 |
...
4. **至少一种拖放路径成功** ✅
   - App 侧（WebView2/生产构建 UI）：合成 drop（扩展真实载荷：text/plain `KANKAN_CARD:` + text/uri-list）→ selectDraggedNoteInput → 导入管线 → 真实解析/下载/OCR/保存，**DROP_IMPORT_OK**
...
## 遗留手动 QA
- 物理鼠标跨窗口拖放（Chrome/Edge → 看看收藏窗口）一次
